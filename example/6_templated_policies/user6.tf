data "vault_policy_document" "user6" {
  ...
  rule {
    path         = "${vault_mount.kv.path}/delete/{{identity.entity.name}}/*"
    capabilities = ["update"]
    description  = "allow delete on kv/{{identity.entity.name}}"
  }

  rule {
    path         = "${vault_mount.kv.path}/undelete/{{identity.entity.name}}/*"
    capabilities = ["update"]
    description  = "allow undelete on kv/{{identity.entity.name}}"
  }

  rule {
    path         = "${vault_mount.kv.path}/destroy/{{identity.entity.name}}/*"
    capabilities = ["update"]
